	...
	function ajax_register(){
		...
		function ajax_register_success() {
			...
			if(!empty($rs['id'])) {
				...
				$this->send_mailer($email, $header, $content);
			...
			function send_mailer($to = false, $subject = false, $message = false) {
				//Example::
				//send_mailer('intelhat@gmail.com', 'ทดสอบ:ลงทะเบียนเสร็จสิ้น', 'ระบบได้รับข้อมูลการสมัครเรียบร้อยแล้ว และจะแจ้งผลการพิจารณาให้ทราบทางอีเมล์ที่ท่านได้ลงทะเบียนไว้ ภายใน 2 วันทำการ ')
